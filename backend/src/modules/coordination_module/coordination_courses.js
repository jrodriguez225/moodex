...
cooordination_courses.getTeacherCourses = (token, userid) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const response = await data_users.getUserCourses(token, userid);
            ...
            for (let coursesIndex = 0; coursesIndex < userCourses.length; coursesIndex++) {
                ...
                const roles = await utils.getCourseUserRoles(token, courseid, userid);
                // mirar si tiene rol de profesor
                const isTeacher = roles.some(role => role.shortname === 'editingteacher' || role.shortname === 'teacher');
                // si lo tiene
                if (isTeacher) {
            ...
            for (let usersIndex = 0; usersIndex < users.length; usersIndex++) {
                ...
                if (isStudent) {
                    ...
                    for (let coursesIndex = 0; coursesIndex < courses.length; coursesIndex++) {
                        ...
                        // obtener los roles del alumno en el curso
                        const roles = await utils.getCourseUserRoles(token, course.id, user.id);
                        // mirar si tiene rol de alumno en el curso
